...
#### **MNIST Dataset**
In this article we will try to explain the fundamental mechanism behind Neural Networks by discussing and showing off the classification of the very well known [MNIST](https://en.wikipedia.org/wiki/MNIST_database) dataset from the *National Institute of Standards and Technology*. It is a set of handwritten digits from $0$ to $9$ in a $28$ by $28$ grid of pixels in which each pixel has got a value between $0$ and $255$, representing a tone of gray ($0$ = black, $255$ = white). In practice, every value is divided by $255$, so that we only work with percentual values between $0$ and $1$.
...
This technique was developed ever since and laid the foundation for the modern *Artificial Intelligence* (*AI*) used today.
Of course, nowadays, there are a lot of different functions that build on these early developed principles, but a finalized version of the *Neural Network* exists to this day and will be discussed in the following.
...
At the end of the *forward pass* the network calculates the *loss* or *cost* which measures how different the networks prediction is from the wanted output. The closer the loss is to $0$, the better the prediction.

**2. The Backward Pass**
Secondly, the network calculates from back to front, the influence of every inner parameter on the prediction using the calculated loss. It figures out which parameters were responsible for it to fail or made the result less precise.

**3. Stochastic Gradient Descent**
Finally, using a method called *Stochastic Gradient Descent (SGD)*, every parameter is changed based on its calculated influence. The goal of this operation is that the same batch would now result in a better prediction. Therefore, if all data samples of the data set follow the same fundamental patterns, the network will slowly learn to make better predictions on the given type of data.

The mathematics behind this training concept will furthermore be discussed.
...
This means that for any layer we are mapping $n$ input dimensions to $m$ output dimensions.
Then, the explained process is continued with each and every layer of the *Neural Network*. The amount of layers and neurons in each layer can be defined freely; there is no right or wrong, but some structures, of course, work better for a specific type of data than others do. The only exceptions are the input layer which needs the exact number of neurons of your measured features, and the output layer which also needs to have the right number of neurons. The reason for that will be addressed in one of the following chapters.
In practice, this means that every *layer* owns a weight matrix that indicates all connections to its previous layer. The data example is then encoded as a vector which is furthermore *passed forward* through the network. Every layer, one after another, multiplies the output vector of the previous layer with its internal weight matrix and passes on its own output vector to the next layer.
By the way, the *weights* are one of the two types of parameters inside the network that change during the training process. They are adjusted so that the network can improve the quality of its predictions.

#### **Adding Bias**
In addition to the weights every neuron contains a so called *bias* that basically is a constant which is added to the weighted sum of the input neurons. This constant ensures that the network is able to shift all inputs and can cover a wider numerical range. Again, we can bundle all biases of one layer $L$ in a vector $\vec{b}$.
